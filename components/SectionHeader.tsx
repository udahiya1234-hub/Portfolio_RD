...
const SectionHeader: React.FC<SectionHeaderProps> = ({ title, subtitle }) => {
  ...
  return (
    <div className="text-center mb-12">
      <h2
        ref={titleRef}
        className={`text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-neutral-50 mb-4 ${titleAnimated ? 'animate-fade-in-up' : 'opacity-0'}`}
      ...
      {subtitle && (
        <p
          ref={subtitleRef}
          className={`text-xl md:text-2xl text-gray-700 dark:text-zinc-300 max-w-2xl mx-auto ${subtitleAnimated ? 'animate-fade-in-up' : 'opacity-0'}`}
      ...
      <div
        ref={lineRef}
        className={`w-24 h-1 bg-sky-600 dark:bg-emerald-500 mx-auto mt-6 rounded-full ${lineAnimated ? 'animate-fade-in-up' : 'opacity-0'}`}
